...
import { Lead, useMerchant } from "../../context/MerchantContext";
...
export function LeadDrawer({ lead, onClose, onUpdateStatus }: LeadDrawerProps) {
  const { showToast } = useMerchant();

  if (!lead) return null;

  // Mock Amazon-style ID for CRM consistency
  ...
  const handleUpdate = (newStatus: Lead["status"]) => {
    onUpdateStatus(lead.id, newStatus);
    if (newStatus === 'Synced') {
      showToast(`WhatsApp sent to ${lead.customer.name}: "Hi ${lead.customer.name.split(' ')[0]}, SimulaFly Merchant has confirmed. We will connect shortly."`);
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-[95vw] md:w-[700px] bg-white shadow-2xl z-50 flex flex-col transform transition-transform duration-300 ease-in-out">
          ...
          <div>
            <div className="flex items-center gap-4 mb-1">
              ...
              <span className={`px-2.5 py-1 text-xs font-bold rounded-full border ${
                lead.status === 'New Lead' ? 'bg-yellow-50 border-yellow-200 text-yellow-700' :
                lead.status === 'Synced' ? 'bg-blue-50 border-blue-200 text-blue-700' :
                lead.status === 'Converted' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' :
                'bg-gray-50 border-gray-200 text-gray-600'
              }`}>
                {lead.status === 'New Lead' ? 'Pending Acceptance' :
                 lead.status === 'Synced' ? 'In Communication' :
                 lead.status === 'Converted' ? 'Payment Received' : 'Lost'}
              </span>
            </div>
            <p className="text-sm text-gray-500 font-medium">
              Reference: <span className="font-mono text-[#1FAF9A]">{lead.id}</span>
            </p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 bg-gray-50/50">
          ...
          {/* Payment Warning - Streamlined */}
          <div className="bg-amber-50/80 border border-amber-200/60 p-3 rounded-lg flex items-center gap-3">
            <svg className="h-5 w-5 text-amber-500 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
            <p className="text-sm text-amber-800">
              <strong className="font-bold">Payment Not Collected.</strong> Contact customer directly to arrange fulfillment and payment.
            </p>
          </div>
              ...
              <div className="text-sm text-gray-800">
                {isNewLead ? (
                  <div className="space-y-3">
                    <p className="font-bold text-xl text-gray-400 select-none">Protected Customer</p>
                    <p className="text-gray-900 font-medium">Location: {lead.customer.city}, India</p>
                  ...
                  <div className="space-y-1.5">
                    <p className="font-bold text-xl text-gray-900">{lead.customer.name}</p>
                    <p className="text-[#1FAF9A] font-semibold">{lead.customer.phone}</p>
            ...
            <div>
              ...
              <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm space-y-3 text-sm">
                <div className="flex justify-between text-gray-600">
                  ...
                  <span className="font-medium">₹{lead.total.toLocaleString('en-IN')}</span>
                </div>
                ...
                <div className="border-t border-gray-100 pt-3 flex justify-between items-center">
                  ...
                  <span className="font-bold text-lg text-gray-900">₹{lead.total.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Requested Items */}
          <div>
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Requested Items</h3>
            <div className="space-y-4">
               {lead.products.map((prod, idx) => (
                 <div key={idx} className="flex items-center gap-4 bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                   <div className={`w-16 h-16 rounded-lg ${prod.img} shrink-0`} />
                   <div className="flex-1">
                     <p className="text-sm font-bold text-gray-900">{prod.name}</p>
                     <p className="text-xs font-mono text-gray-500 mt-0.5">SKU: {prod.sku}</p>
                   </div>
                   <div className="text-right">
                     <p className="text-sm font-bold text-gray-900">₹{prod.price.toLocaleString('en-IN')}</p>
                     <p className="text-xs text-gray-500 mt-0.5">Qty: {prod.qty}</p>
                   </div>
                 </div>
               ))}
            </div>
        ...
        <div className="p-6 bg-white border-t border-gray-100 flex gap-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.02)] z-10">
          {lead.status === 'New Lead' && (
            <button onClick={() => handleUpdate('Synced')} className="flex-1 py-3 bg-[#1FAF9A] hover:bg-[#189986] text-white font-bold text-sm rounded-xl shadow-md shadow-[#1FAF9A]/20 transition-all">
              Confirm Order
            </button>
          )}
          {lead.status === 'Synced' && (
            <button onClick={() => handleUpdate('Converted')} className="flex-1 py-3 bg-[#1FAF9A] hover:bg-[#189986] text-white font-bold text-sm rounded-xl shadow-md shadow-[#1FAF9A]/20 transition-all">
              Mark Payment Received
            </button>
          )}
          {!isNewLead && (
            <a href={`tel:${lead.customer.phone}`} className="flex-1 py-3 bg-white border border-gray-200 text-gray-700 font-bold text-sm rounded-xl hover:bg-gray-50 transition-all text-center block shadow-sm">
              Call Customer
            </a>
          )}
          {(lead.status === 'New Lead' || lead.status === 'Synced') && (
            <button onClick={() => handleUpdate('Lost')} className="px-6 py-3 bg-white border border-gray-200 text-red-600 font-bold text-sm rounded-xl hover:bg-red-50 transition-all shadow-sm">
              Mark Lost
            </button>
          )}
        </div>
